...
def upload_ljson(ctx, inputfile, addannotations, influxbaseurl, influxdatabase, influxtablename, influxuploadbatchsize, grafanaconfigfile):
    ...
    if addannotations:
        ...
        grafana_string = f'{influxtablename}_annotation,sensor=pouch01,type=annotation title="{read_filename}",description="" {round(first_timestamp)}\n'
        grafana_string += f'{influxtablename}_annotation,sensor=pouch01,type=annotation title="{read_filename}",description="" {round(last_timestamp)}\n'
        r = requests.post(f"{influxbaseurl}/write?db={influxdatabase}&precision=u", grafana_string, timeout=10)
        if r.status_code >= 200 and r.status_code <= 299:
